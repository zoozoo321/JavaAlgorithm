...
public class RegExpUtils {
	 ...
	 * @ReferencePage https://gngsn.tistory.com/54
	 * */
	public static String insertUrlPatternInHtmlSource(String htmlSrc, String url, String pcImg, String mobImg) {
		StringBuffer buffer = new StringBuffer();
		
		final String REGEX = "(<img\\b[^>](src=(\"\"|\'\')))|(<area\\b[^>](href=(\"\"|\'\')))"; // result -> [<img src=''][<area href=""][<img src=""][<area href=""]
...
//		
//		while(matcher.find()) {
//			String compare = matcher.group();
//			String lastText = compare.substring(compare.length()-1);
//			if(compare.contains("img src")) {
//				compare = compare.substring(0, compare.length()-1);
//				compare = compare + (cnt == 0 ? pcImg : mobImg) + lastText;
//			} else if(compare.contains("area href")) {
//				compare = compare.substring(0, compare.length()-1);
//				compare = compare + url + lastText;
//			}
//			matcher.appendReplacement(buffer, compare);
//			cnt++;
//		}
//		matcher.appendTail(buffer);	
		...
		for(int i=0; matcher.find(); i++) {
			...
			if(compare.contains("img src")) {
				compare = compare.substring(0, compare.length()-1);
				compare = compare + (i == 0 ? pcImg : mobImg) + lastText;
			} else if(compare.contains("area href")) {
				compare = compare.substring(0, compare.length()-1);
				compare = compare + url + lastText;
			}
